...
# Note: Giao Thua is NOT in this list because lunar month 12 can have 29 or 30 days.
# Giao Thua is detected dynamically using is_giao_thua() function.
LUNAR_HOLIDAYS = [
...
def check_lunar_holiday(lunar_day: int, lunar_month: int, solar_dd: int = None, solar_mm: int = None, solar_yy: int = None) -> Optional[Dict[str, str]]:
    """
    Check if a lunar date matches any lunar holiday.
    Also checks for Giao Thua dynamically (requires solar date).
    
    Args:
        lunar_day: Day in lunar calendar
        lunar_month: Month in lunar calendar
        solar_dd: Solar day (optional, for Giao Thua detection)
        solar_mm: Solar month (optional, for Giao Thua detection)
        solar_yy: Solar year (optional, for Giao Thua detection)
    
    Returns:
        Holiday info dict if found, None otherwise
    """
    # Check fixed lunar holidays first
    ...
    # Check for Giao Thua dynamically (if solar date provided)
    # Giao Thua is the last day of lunar month 12 (can be 29 or 30)
    if lunar_month == 12 and solar_dd and solar_mm and solar_yy:
        if is_giao_thua(solar_dd, solar_mm, solar_yy):
            return {"name": "Giao Thua", "nameVi": "Giao thua"}
...
def is_giao_thua(dd: int, mm: int, yy: int) -> bool:
    """
    Check if a solar date is Giao Thua by checking if tomorrow is Mung 1 Tet.
    This handles both 29-day and 30-day lunar month 12 correctly.
    """
    import datetime
